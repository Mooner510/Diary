...
import kr.mooner510.diary.DateParser.parseDate
import kr.mooner510.diary.DateParser.parseDateTime
...
object EventListener : ListenerAdapter() {
    ...
    override fun onMessageReceived(event: MessageReceivedEvent) {
        ...
        try {
            if (args[0] == ".fetch") {
                event.channel.sendMessage("**일기 새로 고침**: ${Diary.lastFetchTime.toRelative()} 이후 일기 조회")
                    .setMessageReference(event.message)
                    .queue()
                Diary.fetch()
            } else if (raw.startsWith(".diary")) {
                val date = args[1].parseDate()
                Diary.client.newCall(NotionRequest.requestEquals(date)).execute().use { response ->
                ...
                event.channel.sendMessage("**다음 Fetch**: ${Diary.lastFetchTime.toRelative()}")
                    .setMessageReference(event.message)
                    .queue()
            }
            ...
            if (args[0] == ".reset") {
                ...
                event.channel.sendMessage("**타이머 초기화**\n다음 Fetch: ${Diary.lastFetchTime.plusMinutes(10).toDiscordDateTime()}")
                    .setMessageReference(event.message)
                    .queue()
                Diary.reset()
                ...
                try {
                    ...
                    Diary.lastFetchTime = date
                    event.channel.sendMessage("**날짜 재설정**: ${Diary.lastFetchTime.toDiscordDateTime()}에 마지막으로 Fetch한 것으로 변경됨")
                        .setMessageReference(event.message)
                        .queue()
                } catch (_: Exception) {
                    try {
                        val date = args[1].parseDate()
                        Diary.lastFetchTime = date.atStartOfDay()
                        event.channel.sendMessage("**날짜 재설정**: ${Diary.lastFetchTime.toDiscordDateTime()}에 마지막으로 Fetch한 것으로 변경됨")
                            .setMessageReference(event.message)
                            .queue()
                    } catch (_: Exception) {
