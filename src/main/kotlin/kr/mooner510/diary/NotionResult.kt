...
object NotionResult {
    fun packaging(obj: JSONObject, statusColor: Boolean = false): MessageEmbed? {
        ...
        val isNew = LocalDateTime.parse(obj.getString("created_time"), Diary.formatter) > Diary.lastFetchTime

        return Diary.client.newCall(NotionRequest.requestBlocks(obj.getString("id"))).execute().use { response2 ->
